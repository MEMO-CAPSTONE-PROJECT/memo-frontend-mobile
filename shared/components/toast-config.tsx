...
export const toastConfig: ToastConfig = {
    ...
    alert: ({ text1, text2, onPress, props }) => (
        <TouchableOpacity style={{ width: "auto", height: "auto" }} onPress={onPress}>
            <View className="flex-1 flex-col items-center bg-system-white rounded-sm p-lg gap-x-lg">
                {props?.icon}
                <View className="w-full flex-col flex-wrap justify-center">
                    <Text className="font-kanit-bold text-body text-body-1 text-center">{text1}</Text>
                    <Text className="font-kanit-medium text-caption-1 text-body-2">{text2}</Text>
                </View>
            </View>
        </TouchableOpacity>
