import { AchievementByIdResponse } from "@/hooks/achievement/useAchievementQuery"

export const mockUseTeacherAchievements = {
    data: {
        "achievementTeacher": [
            {
                "id": "1",
                "teacherId": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "images": [
                    {
                        "fileEndPoint": "https://upload.wikimedia.org/wikipedia/commons/4/47/PNG_transparency_demonstration_1.png",
                    },
                    {
                        "fileEndPoint": "https://upload.wikimedia.org/wikipedia/commons/6/6a/JavaScript-logo.png",
                    }
                ],
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },
            {
                "id": "1",
                "teacherId": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "images": [
                    {
                        "fileEndPoint": "https://upload.wikimedia.org/wikipedia/commons/4/47/PNG_transparency_demonstration_1.png",
                    },
                    {
                        "fileEndPoint": "https://upload.wikimedia.org/wikipedia/commons/6/6a/JavaScript-logo.png",
                    }
                ],
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },
            {
                "id": "1",
                "teacherId": "2",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },{
                "id": "1",
                "teacherId": "2",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            }
        ]
    }
}

export const mockUseStudentAchievements = {
    data: {
        "achievementStudent": [
            {
                "id": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },
            {
                "id": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },
            {
                "id": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            },{
                "id": "1",
                "name": "แข่งเพชรยอดมงกุฎ",
                "src": "path/to/image",
                "people": {
                    "current": 1,
                    "max": 10
                },
                "sections": {
                    "startDate": "2024-09-01T12:00:00Z",
                    "endDate": "2024-09-02T23:59:59.999Z",
                    "organizer": "เรนู ทำงานหนัก"
                },
                "points": [
                    {
                        "id": "volunteer",
                        "excellent": 10,
                        "normal": 5,
                        "details": [
                            {
                                "id": "volunteer",
                                "type": "จิตอาสา",
                                "color": "blue"
                            }
                        ]
                    },
                    {
                        "id": "self-confident",
                        "excellent": 7,
                        "normal": 3,
                        "details": [
                            {
                                "id": "self-confident",
                                "type": "ความกล้าแสดงออก",
                                "color": "purple"
                            }
                        ]
                    }
                ]
            }
        ]
    }
}

export const mockUseTeacherAchievementById: AchievementByIdResponse = {
    "data": {
        "achievementTeacher": {

            "id": "1",
            "teacherId": "1",
            "name": "แข่งเพชรยอดมงกุฎ",
            "src": "path/to/image",
            "people": {
                "current": 1,
                "max": 10
            },
            "sections": {
                "startDate": "2024-09-01T12:00:00Z",
                "endDate": "2024-09-02T23:59:59.999Z",
                "organizer": "เรนู ทำงานหนัก"
            },
            "points": [
                {
                    "id": "volunteer",
                    "excellent": 10,
                    "normal": 5,
                    "details": [
                        {
                            "id": "volunteer",
                            "type": "จิตอาสา",
                            "color": "blue"
                        }
                    ]
                },
                {
                    "id": "self-confident",
                    "excellent": 7,
                    "normal": 3,
                    "details": [
                        {
                            "id": "self-confident",
                            "type": "ความกล้าแสดงออก",
                            "color": "purple"
                        }
                    ]
                }
            ],
            "description": "รับตัวแทน แข่งวิทย์ 3 คน  แข่ง ณ โรงยิมของโรงเรียน\nสิ่งของที่ต้องเตรียม\n- ผ้ากันเปื้อน\n- ยาสีฟัน"

        }
    }
}

export const mockUseCreateTeacherAchievement = {
    "teacherId": 20,
    "name": "แข่งวิ้งเปรี้ยว",
    "description": "รับตัวแทน 10 คน ซ้อมทุกวันพุธ หลัง 18.00 น.",
    "amount": 10,
    "startDate": "2024-11-24",
    "endDate": "2024-12-24",
    "points": [
        {
            "type": "volunteer",
            "excellent": 5,
            "normal": 2
        },
        {
            "type": "self-confident",
            "excellent": 9,
            "normal": 6
        }
    ]
}

export const mockUseStudentAchievementById = {
    "data": {
        "achievementStudent": {
            "id": "1",
            "teacherId": "1",
            "name": "แข่งเพชรยอดมงกุฎ",
            "src": "path/to/image",
            "people": {
                "current": 1,
                "max": 10
            },
            "sections": {
                "startDate": "2024-09-01T12:00:00Z",
                "endDate": "2024-09-02T23:59:59.999Z",
                "organizer": "เรนู ทำงานหนัก"
            },
            "points": [
                {
                    "id": "volunteer",
                    "excellent": 10,
                    "normal": 5,
                    "details": [
                        {
                            "id": "volunteer",
                            "type": "จิตอาสา",
                            "color": "blue"
                        }
                    ]
                },
                {
                    "id": "self-confident",
                    "excellent": 7,
                    "normal": 3,
                    "details": [
                        {
                            "id": "self-confident",
                            "type": "ความกล้าแสดงออก",
                            "color": "purple"
                        }
                    ]
                }
            ],
            "description": "รับตัวแทน แข่งวิทย์ 3 คน  แข่ง ณ โรงยิมของโรงเรียน\nสิ่งของที่ต้องเตรียม\n- ผ้ากันเปื้อน\n- ยาสีฟัน"

        }
    }
}

export const mockUseCreateStudentAchievement = {
    "name": "แข่งวิ้งเปรี้ยว",
    "description": "รับตัวแทน 10 คน ซ้อมทุกวันพุธ หลัง 18.00 น.",
    "amount": 10,
    "startDate": "2024-11-24",
    "endDate": "2024-12-24",
    "points": [
        {
            "type": "volunteer",
            "excellent": 5,
            "normal": 2
        },
        {
            "type": "self-confident",
            "excellent": 9,
            "normal": 6
        }
    ]
}